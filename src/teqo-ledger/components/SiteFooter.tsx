    ...
    <footer id="careers" className="scroll-mt-20 bg-[hsl(var(--deep))] px-6 pb-8 pt-16 text-white">
      <div className="mx-auto max-w-7xl">
        <div className="grid grid-cols-2 gap-8 border-t border-white/10 pt-12 sm:grid-cols-3 lg:grid-cols-6">
          <div className="col-span-2">
            <div className="flex items-baseline gap-2">
              <span className="text-xl font-bold tracking-tight text-brand-foreground">TEQO</span>
          ...
          {FOOTER_COLUMNS.map((col) => (
            <div key={col.heading}>
              <h4 className="text-xs font-semibold uppercase tracking-wider text-white/40">{col.heading}</h4>
              <ul className="mt-4 flex flex-col gap-2.5">
                {col.links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-sm text-white/80 transition-colors hover:text-green">
                      {link}
                    </a>
                  </li>
                ))}
        ...
        <div className="mt-14 flex flex-col gap-3 border-t border-white/10 pt-6 text-xs text-white/40 sm:flex-row sm:items-center sm:justify-between">
          ...
          <a href="#contact" className="font-medium text-white/70 hover:text-green">
            Get in touch →
          </a>
        </div>
      </div>
